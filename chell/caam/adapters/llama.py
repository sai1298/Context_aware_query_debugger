...
if HAS_TORCH:
    ...
    class _LlamaCAAMAttn(nn.Module):
        """Wrapper that replaces a Llama self-attention module with CAAM."""
        ...
        def __init__(
            ...
            self.out_proj = original_attn.o_proj
...
def patch_llama(model: object, context_dim: int, scale: float) -> object:
    """Patch a Llama model with ContextAwareAttention.

    Iterates over ``model.model.layers`` and replaces each ``layer.self_attn``
    with a ``_LlamaCAAMAttn`` wrapper that adds a context bias to QK^T logits.

    Args:
        model: A ``transformers.AutoModelForCausalLM`` Llama instance.
    ...
    """
    if not HAS_TORCH:
        raise ImportError("patch_llama requires PyTorch. Install it with: pip install torch")

    for layer in model.model.layers:  # type: ignore[union-attr]
        original_attn = layer.self_attn
        layer.self_attn = _LlamaCAAMAttn(original_attn, context_dim=context_dim, scale=scale)
