...
if HAS_TORCH:
    ...
    class _Qwen2CAAMAttn(nn.Module):
        """Wrapper that replaces a Qwen2 self-attention module with CAAM."""
...
def patch_qwen2(model: object, context_dim: int, scale: float) -> object:
    """Patch a Qwen2 model with ContextAwareAttention.

    Iterates over ``model.model.layers`` and replaces each ``layer.self_attn``
    with a ``_Qwen2CAAMAttn`` wrapper that adds a context bias to QK^T logits.

    Args:
        model: A ``transformers.AutoModelForCausalLM`` Qwen2 instance.
    ...
    """
    if not HAS_TORCH:
        raise ImportError("patch_qwen2 requires PyTorch. Install it with: pip install torch")

    for layer in model.model.layers:  # type: ignore[union-attr]
        original_attn = layer.self_attn
        layer.self_attn = _Qwen2CAAMAttn(original_attn, context_dim=context_dim, scale=scale)
